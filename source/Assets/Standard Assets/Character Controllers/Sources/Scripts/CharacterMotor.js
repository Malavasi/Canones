...
class CharacterMotorMovement {
	...
	var maxForwardSpeed : float = 320.0;
	var maxSidewaysSpeed : float = 320.0;
	var maxBackwardsSpeed : float = 320.0;
	
	// Curve for multiplying speed based on slope (negative = downwards)
	var slopeSpeedMultiplier : AnimationCurve = AnimationCurve(Keyframe(-90, 1), Keyframe(0, 1), Keyframe(90, 0));
	
	// How fast does the character change speeds?  Higher is faster.
	var maxGroundAcceleration : float = 320.0;
	var maxAirAcceleration : float = 320.0;
...
class CharacterMotorJumping {
	...
	var baseHeight : float = 150.0;
	
	// We add extraHeight units (meters) on top when holding the button down longer while jumping
	var extraHeight : float = 170.1;
